  ...
  <div class="flex flex-col">
    <div class="flex flex-row non-drag">
      ...
      <button
          ...
          @click="initGame(globalWidth, globalHeight, globalBombs)">Restart
      </button>
    </div>
    <div>
...
const globalWidth = computed(() => props.width)
const globalHeight = computed(() => props.height)
const globalBombs = computed(() => props.bombs)

const flags: Ref<number> = ref(globalBombs.value)
...
const cellCount: Ref<number> = ref(globalWidth.value * globalHeight.value - globalBombs.value)
...
function dimCell(cell: CellType) {
  if (moves.value === 0) {
    ...
    generateBombs(globalWidth.value, globalHeight.value, globalBombs.value)
    grid[cell.row][cell.col].bomb = false
    generateNumbers(globalWidth.value, globalHeight.value)
  }
...
onBeforeMount(() => {
  initGame(globalWidth.value, globalHeight.value, globalBombs.value)
})
</script>
